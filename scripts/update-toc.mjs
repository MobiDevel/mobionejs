...
function extractHeadings(lines, minLevel = 2, maxLevel = 3) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const text = m[2].trim();
    if (text.toLowerCase() === 'table of contents' || text === 'Quick Links' || text.startsWith('Example')) continue;
...
async function updateReadmeToc() {
    ...
    if (quickLinksIdx !== -1) {
      ...
      while (cursor < md.length) {
        ...
        if (line.startsWith('|')) inTable = true;
          ...
          return md.slice(0, insertionPoint) + `\n${START_MARKER}\n${END_MARKER}` + md.slice(insertionPoint);
  ...
  const newContent = content.slice(0, startIdx + START_MARKER.length) + region + content.slice(endIdx);
